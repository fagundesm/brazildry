...
source("MEE3_070_sm_NOa/MEE3_070_sm_NicheFunctions.txt")
...
# ??????????????????????????????????????????????????
A.df1 <- read.csv("6meses.csv",T)
A.df <- select(A.df1,id, species, SLA,altura_aerea_cm,t_dens,t_capac_arm_casca,r_capac_arm_casca,comp_raiz_cm,r_dens,SRA)
# ?????????????????????????????????????????????????? 
...
# The following command goes with the ExampleA.txt data set:
vartypes <- c("cts","cts","cts","cts","cts","cts","cts","cts")
# ??????????????????????????????????????????????????
...
for (rr in 1:replic)
{
  ...
  for (vv in 1:no.vars)
  {
    ...
    if (vartypes[vv] == "meas")
      pseudo.no.array[,,vv,rr] <- no.cts.fn(temp.df$species,log.fn(y))
    if (vartypes[vv] == "pcent")
...
#Matriz de estimativa de sobreposição de nicho para cada trait
attributes(NOa.results$NOestimates) # PERGUNTAR PARA O ALEXANDRE 
#SE A NOA.ESTIMATES É A MEDIDA DE SOBREPOSIÇAO CERTA PARA USAR E SE A MEDIA DESSAS MEDIDAS É O RECOMENDABEL.
apply(NOa.results$NOestimates , c(1,2), mean) #média conservando as dimensoes 1 e 2 (especies) USAR ESTE VALOR
NOa.results$NOestimates[, ,"dens_mad"]
...
write.table(NOa.results$NOestimates, file = "1-NO p variavel - estimates.csv", sep = ";", col.names = T)
write.table(NOa.results$separate.pvalues, file = "2-NO p variavel - p values.csv", sep = ";", col.names = T)
write.table(NOa.results$separate.cluster.pvalues, file = "3-NO separate cluster values.csv", sep = ";", col.names = T)
write.table(NOa.results$separate.differentiated.pvalues, file = "4-NO separate diff p values.csv", sep = ";", col.names = T)
write.table(NOa.results$ests.overall, file = "5-NO tests overall.csv", sep = ";", col.names = T)
write.table(NOa.results$ests.overall.sd, file = "6-NO tests overall sd.csv", sep = ";", col.names = T)
write.table(NOa.results$overall.pvalues, file = "7-NO overall p values.csv", sep = ";", col.names = T)
NOa.results$overall.cluster.pvalues
NOa.results$overall.differentiated.pvalues

###############

# NMDS

teste = 1 - NOa.results$ests.overall
nmds = metaMDSiter(as.dist(teste),k=3, zerodist="add")
plot(nmds)